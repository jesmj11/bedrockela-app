// Parse Moby Dick into unit card JSON
const fs = require('fs');

const chaptersText = fs.readFileSync('book-data/moby-chapters-raw.txt', 'utf8');
const qvText = fs.readFileSync('book-data/moby-qv-raw.txt', 'utf8');
...
const unitCard = {
  ...
  description: "A YA adaptation exploring obsession, identity, and the limits of human ambition",
  chapters: []
};
...
// Save unit card
fs.writeFileSync(
  'book-data/moby-dick-unit-card.json',
  JSON.stringify(unitCard, null, 2)
);

console.log(`\n✅ Moby Dick Unit Card Created!`);
...
console.log(`   File: book-data/moby-dick-unit-card.json\n`);
