...
// 9 books for 6th grade (Days 1-180)
const books = [
  { file: 'tom-sawyer-complete-unit-card.json', dayStart: 1, dayEnd: 20, title: 'The Adventures of Tom Sawyer' },
  { file: 'twenty-thousand-leagues-complete-unit-card.json', dayStart: 21, dayEnd: 40, title: 'Twenty Thousand Leagues Under the Sea' },
  { file: 'robin-hood-complete-unit-card.json', dayStart: 41, dayEnd: 60, title: 'The Merry Adventures of Robin Hood' },
  { file: 'swiss-family-complete-unit-card.json', dayStart: 61, dayEnd: 80, title: 'The Swiss Family Robinson' },
  { file: 'journey-complete-unit-card.json', dayStart: 81, dayEnd: 100, title: 'Journey to the Center of the Earth' },
  { file: 'connecticut-yankee-complete-unit-card.json', dayStart: 121, dayEnd: 140, title: 'A Connecticut Yankee in King Arthur\'s Court' },
  { file: 'princess-goblin-complete-unit-card.json', dayStart: 141, dayEnd: 160, title: 'The Princess and the Goblin' },
  { file: 'odyssey-complete-unit-card.json', dayStart: 161, dayEnd: 180, title: 'The Odyssey' }
];

// Norse Mythology (Days 101-120) - will use existing mythology unit from 5th grade or create simple placeholder
  ...
  pages.push(`
    <div class="lesson-page" data-page="1">
      <div class="page-content title-page">
        ...
        <p class="lesson-focus"><strong>6th Grade ELA</strong></p>
      </div>
    </div>
  `);
...
<head>
  ...
  <title>6th Grade Day ${day} - BedrockELA</title>
...
// MAIN BUILD LOOP
console.log('\n🏗️  Building ALL 180 6th Grade Lessons!\n');

let totalBuilt = 0;

// First, build Days 101-120 (Norse Mythology) with placeholder
console.log('\n📚 Building Norse Mythology (Days 101-120) - Placeholder Unit...');
for (let day = 101; day <= 120; day++) {
  if (!fs.existsSync('curriculum/grade6')) {
    fs.mkdirSync('curriculum/grade6', { recursive: true });
  }
  ...
  if (relativeDay % 5 === 0) {
    ...
    const simpleAssessment = buildSimplePlaceholder(day, weekNumber, 'Norse Mythology Assessment');
    const filename = `curriculum/grade6/6th-grade-lesson-${day.toString().padStart(3, '0')}.html`;
    fs.writeFileSync(filename, simpleAssessment);
    console.log(`✅ Day ${day}: Week ${weekNumber} Assessment`);
    totalBuilt++;
  } else {
    // Regular
    const simpleLesson = buildSimplePlaceholder(day, 0, `Norse Mythology - Chapter ${relativeDay}`);
    const filename = `curriculum/grade6/6th-grade-lesson-${day.toString().padStart(3, '0')}.html`;
    fs.writeFileSync(filename, simpleLesson);
    console.log(`✅ Day ${day}: Norse Mythology Chapter ${relativeDay}`);
    totalBuilt++;
  }
}

// Helper for placeholder lessons
function buildSimplePlaceholder(day, weekNum, title) {
  return `<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>6th Grade Day ${day}</title></head>
<body style="font-family:sans-serif;max-width:800px;margin:50px auto;padding:20px;">
<h1>Day ${day}: ${title}</h1>
<p>This lesson is being prepared. Check back soon!</p>
<p style="color:#666;margin-top:40px;">BedrockELA - 6th Grade</p>
</body></html>`;
}
...
console.log(`\n🎉 Built ${totalBuilt} 6th Grade lessons!`);
console.log(`📂 Location: curriculum/grade6/\n`);
