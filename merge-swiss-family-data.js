#!/usr/bin/env node
/**
 * MERGE SWISS FAMILY ROBINSON DATA INTO COMPLETE UNIT CARD
 * 
...
console.log('📦 Merging Swiss Family Robinson data into complete unit card...\n');

// Load both files
const unitCard = JSON.parse(fs.readFileSync('./book-data/swiss-family-unit-card.json', 'utf-8'));
const curriculum = JSON.parse(fs.readFileSync('./book-data/swiss-family-curriculum-complete.json', 'utf-8'));
...
fs.writeFileSync('./book-data/swiss-family-complete-unit-card.json', JSON.stringify(completeUnitCard, null, 2), 'utf-8');

console.log('✅ Complete unit card created!');
console.log(`📄 Saved to: book-data/swiss-family-complete-unit-card.json\n`);
