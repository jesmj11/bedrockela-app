...
// Load data
const chapters = require('./book-data/tom-sawyer-chapters.json');
...
// Comprehensive vocabulary for all 24 days
const vocabularyData = {
  1: ['melancholy', 'tranquil', 'endeavor'],
  2: ['reluctant', 'commence', 'vigilant'],
  3: ['dismal', 'sagacity', 'revelation'],
  4: ['deliberate', 'diligent', 'perplexed'],
  6: ['contemplated', 'dreary', 'inviting'],
  7: ['discord', 'jubilant', 'venture'],
  8: ['solemn', 'gratitude', 'astounding'],
  9: ['integrity', 'prosperity', 'adversity'],
  11: ['fervent', 'eloquent', 'reverence'],
  12: ['compassion', 'benevolent', 'persevere'],
  13: ['melancholy', 'perilous', 'steadfast'],
  14: ['candid', 'disposition', 'earnest'],
  16: ['conviction', 'profound', 'substantial'],
  17: ['audacious', 'formidable', 'resilient'],
  18: ['conspicuous', 'elaborate', 'imminent'],
  19: ['tranquility', 'vex', 'wrath'],
  21: ['apprehension', 'indignant', 'reproach'],
  22: ['desolate', 'endure', 'refuge'],
  23: ['exuberant', 'haggard', 'manifold'],
  24: ['providence', 'reconcile', 'sublime'],
  26: ['bewildered', 'desolation', 'forlorn'],
  27: ['fortitude', 'ingenious', 'perseverance'],
  28: ['calamity', 'dismayed', 'treacherous'],
  29: ['consolation', 'deliverance', 'triumphant']
};
...
// Copy Day 4 pattern for remaining days (6, 7, 8, 9, etc.)
for (let day = 6; day <= 29; day++) {
  if (day === 5 || day === 10 || day === 15 || day === 20 || day === 25 || day === 30) continue;
  
  const chapterNum = Math.ceil(day / 5) * 2 - (day % 5 === 4 || day % 5 === 3 || day % 5 === 2 || day % 5 === 1 ? 1 : 0);
  const weekNum = Math.ceil(day / 5);
  
  infoTextTopics[day] = {
    title: `Missouri in the 1840s - Part ${Math.floor(day/4)}`,
    text: `This informational text would cover aspects of life in 1840s Missouri, including social customs, economic conditions, education, and daily life along the Mississippi River. Content specific to Chapter ${chapterNum} themes would be included here.

The Adventures of Tom Sawyer provides a window into American life during a pivotal period in the nation's history. Through Tom's adventures, readers learn about the values, challenges, and dreams of people living in small-town America during the mid-19th century.
...
console.log('📚 Generating 6th Grade Bedrock Spine lessons (Tom Sawyer)...\n');
...
console.log('\n✅ Generated 24 complete 6th grade Bedrock Spine lessons!');
console.log('📊 Structure: 11 pages per lesson');
console.log('📖 Tom Sawyer: 24 chapters across 6 weeks');
...
function generateBedrockSpineLesson(day) {
  ...
  const lessonNumber = day - Math.floor((day-1) / 5);
  const chapterNum = lessonNumber;
  ...
  const vocab = vocabularyData[day] || ['word1', 'word2', 'word3'];
  ...
  // Generate vocab game
  const vocabGame = generateVocabMatchingGame(vocab, day);
  
  // Writing or Journal
  const writingJournalPage = isOddDay ? generateWritingPage(day, chapter.title) : generateJournalPage(day, chapter.title);
  
  const html = generateLessonHTML(day, week, chapterNum, chapter, vocab, vocabGame, storyParts, skillContent, infoText, writingJournalPage, isOddDay);
...
function generateVocabMatchingGame(words, day) {
  return `
                // Page 4: Vocabulary Matching Game
                {
                    render: () => {
                        ...
                        const definitions = [
                            'Definition for ${words[0]} (look up in dictionary)',
                            'Definition for ${words[1]} (look up in dictionary)',
                            'Definition for ${words[2]} (look up in dictionary)'
                        ];
...
function generateLessonHTML(day, week, chapterNum, chapter, vocab, vocabGame, storyParts, skillContent, infoText, writingJournalPage, isOddDay) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <script>
        const lesson${day}Config = {
            ...
            pages: [
                            ...
                            <div class="lesson-page-card content-page">
                                ...
                                <div style="margin-bottom: 30px; padding: 20px; background: white; border-radius: 12px; border: 2px solid #8B4513;">
                                    <div style="margin-bottom: 12px;">
                                        <span style="font-size: 28px; font-weight: 800; color: #305853;">${vocab[0]}</span>
                                    </div>
                                    ...
                                    <textarea 
                                      ...
                                      oninput="updateWordCount('vocab-1', 'vocab-count-1', 10)"
                                      onpaste="return false"
                                      placeholder="Type the definition here (minimum 10 words, no pasting)..."
                                    ></textarea>
                                    <div style="display: flex; justify-content: space-between; margin-top: 8px;">
                                      <span id="vocab-count-1" style="font-size: 13px; color: #666;">0 / 10 words</span>
                                      <span id="vocab-count-1-status" style="font-size: 13px; font-weight: 600;"></span>
                                    </div>
                                </div>
                                
                                <div style="margin-bottom: 30px; padding: 20px; background: white; border-radius: 12px; border: 2px solid #8B4513;">
                                    <div style="margin-bottom: 12px;">
                                        <span style="font-size: 28px; font-weight: 800; color: #305853;">${vocab[1]}</span>
                                    </div>
                                    <p style="font-size: 14px; color: #666; margin-bottom: 10px;">
                                        Look up this word in a dictionary, then write the definition in your own words:
                                    </p>
                                    <textarea 
                                      id="vocab-2"
                                      style="width: 100%; min-height: 80px; padding: 10px; border: 2px solid #305853; border-radius: 8px; font-family: inherit; font-size: 16px;"
                                      oninput="updateWordCount('vocab-2', 'vocab-count-2', 10)"
                                      onpaste="return false"
                                      placeholder="Type the definition here (minimum 10 words, no pasting)..."
                                    ></textarea>
                                    <div style="display: flex; justify-content: space-between; margin-top: 8px;">
                                      <span id="vocab-count-2" style="font-size: 13px; color: #666;">0 / 10 words</span>
                                      <span id="vocab-count-2-status" style="font-size: 13px; font-weight: 600;"></span>
                                    </div>
                                </div>
                                
                                <div style="margin-bottom: 30px; padding: 20px; background: white; border-radius: 12px; border: 2px solid #8B4513;">
                                    <div style="margin-bottom: 12px;">
                                        <span style="font-size: 28px; font-weight: 800; color: #305853;">${vocab[2]}</span>
                                    </div>
                                    <p style="font-size: 14px; color: #666; margin-bottom: 10px;">
                                        Look up this word in a dictionary, then write the definition in your own words:
                                    </p>
                                    <textarea 
                                      id="vocab-3"
                                      style="width: 100%; min-height: 80px; padding: 10px; border: 2px solid #305853; border-radius: 8px; font-family: inherit; font-size: 16px;"
                                      oninput="updateWordCount('vocab-3', 'vocab-count-3', 10)"
                                      onpaste="return false"
                                      placeholder="Type the definition here (minimum 10 words, no pasting)..."
                                    ></textarea>
                                    <div style="display: flex; justify-content: space-between; margin-top: 8px;">
                                      <span id="vocab-count-3" style="font-size: 13px; color: #666;">0 / 10 words</span>
                                      <span id="vocab-count-3-status" style="font-size: 13px; font-weight: 600;"></span>
                                    </div>
                                </div>
                ...
                // Page 8: Comprehension (3 questions)
                {
                    render: () => \`
                        <div class="lesson-page-card content-page">
                            ...
                            <div style="margin-bottom: 30px; padding: 20px; background: white; border-radius: 12px; border: 2px solid #8B4513;">
                                <p style="font-weight: 600; margin-bottom: 12px;">1. What happened at the beginning of this chapter?</p>
                                <textarea 
                                  id="comp-1"
                                  style="width: 100%; min-height: 100px; padding: 15px; border: 2px solid #305853; border-radius: 8px; font-size: 16px; line-height: 1.6;"
                                  placeholder="Write your answer here..."
                                ></textarea>
                            </div>
                            
                            <div style="margin-bottom: 30px; padding: 20px; background: white; border-radius: 12px; border: 2px solid #8B4513;">
                                <p style="font-weight: 600; margin-bottom: 12px;">2. What did Tom do in this chapter, and why?</p>
                                <textarea 
                                  id="comp-2"
                                  style="width: 100%; min-height: 100px; padding: 15px; border: 2px solid #305853; border-radius: 8px; font-size: 16px; line-height: 1.6;"
                                  placeholder="Write your answer here..."
                            ...
                            <div style="padding: 20px; background: white; border-radius: 12px; border: 2px solid #8B4513;">
                                <p style="font-weight: 600; margin-bottom: 12px;">3. How did this chapter end? What do you think will happen next?</p>
                                <textarea 
                                  id="comp-3"
                                  style="width: 100%; min-height: 100px; padding: 15px; border: 2px solid #305853; border-radius: 8px; font-size: 16px; line-height: 1.6;"
                                  placeholder="Write your answer here..."
